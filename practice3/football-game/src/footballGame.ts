...
class Football {
    ...
    executeGameAction() {
        ...
        const passPosition: FieldPosition = [generateRandomNumber(100), generateRandomNumber(100)];
    ...
    updateScore(player: Player, points: number, event: string) {
        player.score += points;
...
function simulateShotOnGoal(): boolean {
    const result = generateRandomNumber(3);
    return result === 0;
}

function simulatePass(player: Player, fieldPosition: FieldPosition): boolean {
